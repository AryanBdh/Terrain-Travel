...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name) && !empty($description) && !empty($price)) {
        ...
        header("Location: /travel/agency/packages");
        exit;
    }
}
        ...
        <table>
            ...
            <?php foreach ($packages as $index => $package): ?>
                ...
                <tr data-page="<?= $pageIndex ?>" style="display: <?= $pageIndex === 1 ? 'table-row' : 'none' ?>;">
                    ...
                    <td>
                        <a href="/travel/agency/packages?package_id=<?= htmlspecialchars($package['package_id']); ?>">Edit</a>
                        <a href="/travel/agency/deletePackage?id=<?= htmlspecialchars($package['package_id']); ?>" onclick="return confirm('Are you sure you want to delete this package?');">Delete</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
